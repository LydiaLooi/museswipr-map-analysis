...
class MapPatterns:
    ...
    def _handle_first_other_group(self, current_other, pattern: Pattern):
        ...
        """
        if len(self.patterns) > 1 and len(pattern.segments) == 1 and self.segment_is_interval(pattern.segments[0]):
            current_other = None
        else:
    ...
    def identify_patterns(self, segments_list: List[Segment], merge_other: bool = True) -> List[Pattern]:
        for i in range(0, len(segments_list)):
                ...
                if _added == True:
                    ...
                    if group.is_appendable():
                        ...
                        if len(group.segments) < len(self.other_pattern.segments):
                            ...
                            other_group = OtherPattern(OTHER, self.other_pattern.segments[:-len(group.segments)])
                            self.patterns.append(other_group)

                        added = True
                        group_copy = group.__class__(group.group_name, group.segments, group.start_sample, group.end_sample)
            ...
            if not added: 
                ...
                self.patterns.append(OtherPattern(OTHER, self.other_pattern.segments, self.other_pattern.start_sample, self.other_pattern.end_sample))
                self.other_pattern.reset_group(previous_segment, current_segment) # reset OtherGroup
                # Reset all groups with current pattern.
                for group in self.groups:
                    group.reset_group(previous_segment, current_segment)
        ...
        for last_check_pattern in self.groups:
            if last_check_pattern.is_appendable():
                last_pattern_copy = last_check_pattern.__class__(last_check_pattern.group_name, last_check_pattern.segments, last_check_pattern.start_sample, last_check_pattern.end_sample)
        ...
        if len(self.other_pattern.segments) > 0:
            ...
            if len(self.patterns) == 0 or not (len(self.other_pattern.segments) == 1 and self.segment_is_interval(self.other_pattern.segments[0])):
                last_pattern_copy = OtherPattern(OTHER, self.other_pattern.segments, self.other_pattern.start_sample, self.other_pattern.end_sample)
                self.patterns.append(last_pattern_copy)
