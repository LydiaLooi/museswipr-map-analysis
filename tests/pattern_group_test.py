...
def test_even_circle_with_intervals_start_and_end():
    patterns = [
        ...
        Segment(
            SWITCH,
            [Note(0, 10 * DEFAULT_SAMPLE_RATE), Note(1, 10.1 * DEFAULT_SAMPLE_RATE)],
        ),
        Segment(
            TWO_STACK,
            [Note(1, 10.1 * DEFAULT_SAMPLE_RATE), Note(1, 10.2 * DEFAULT_SAMPLE_RATE)],
        ),
        Segment(
            SWITCH,
            [Note(1, 10.2 * DEFAULT_SAMPLE_RATE), Note(0, 10.3 * DEFAULT_SAMPLE_RATE)],
        ),
        Segment(
            TWO_STACK,
            [Note(0, 10.3 * DEFAULT_SAMPLE_RATE), Note(0, 10.4 * DEFAULT_SAMPLE_RATE)],
        ),
        Segment(
            SWITCH,
            [Note(0, 10.4 * DEFAULT_SAMPLE_RATE), Note(1, 10.5 * DEFAULT_SAMPLE_RATE)],
        ),
        Segment(
            TWO_STACK,
            [Note(1, 10.5 * DEFAULT_SAMPLE_RATE), Note(1, 10.6 * DEFAULT_SAMPLE_RATE)],
        ),
...
def test_invalid_even_circles_pattern():
    patterns = [
        ...
        Segment(
            TWO_STACK,
            [Note(0, 0.2 * DEFAULT_SAMPLE_RATE), Note(1, 0.4 * DEFAULT_SAMPLE_RATE)],
            0,
        ),
        Segment(
            SWITCH,
            [Note(1, 0.4 * DEFAULT_SAMPLE_RATE), Note(1, 0.6 * DEFAULT_SAMPLE_RATE)],
            0,
        ),
        Segment(
            MED_INTERVAL,
            [Note(1, 0.6 * DEFAULT_SAMPLE_RATE), Note(1, 1.2 * DEFAULT_SAMPLE_RATE)],
            0,
        ),
    ]
...
def test_even_circles_pattern():
    patterns = [
        Segment(
            TWO_STACK,
            [
                Note(0, 21.42 * DEFAULT_SAMPLE_RATE),
                Note(0, 21.60 * DEFAULT_SAMPLE_RATE),
            ],
            0,
        ),
        Segment(
            ...
            [
                Note(0, 21.60 * DEFAULT_SAMPLE_RATE),
                Note(0, 21.78 * DEFAULT_SAMPLE_RATE),
            ],
            0,
        ),
        Segment(
            ...
            [
                Note(0, 21.78 * DEFAULT_SAMPLE_RATE),
                Note(0, 21.96 * DEFAULT_SAMPLE_RATE),
            ],
            0,
        ),
    ]
...
def test_even_circles_have_different_time_diffs_not_even_circles():
    patterns = [
        ...
        Segment(
            SWITCH,
            [Note(0, 0.2 * DEFAULT_SAMPLE_RATE), Note(1, 0.3 * DEFAULT_SAMPLE_RATE)],
            0,
        ),  # Switch is not same time by 0.1s
        Segment(
            TWO_STACK,
            [Note(1, 0.3 * DEFAULT_SAMPLE_RATE), Note(1, 0.5 * DEFAULT_SAMPLE_RATE)],
            0,
        ),
    ]
    groups = MapPatterns().identify_patterns(patterns)
...
def test_even_circles_same_time_diff_with_intervals_with_diff_time_diff():
    patterns = [
        ...
        Segment(
            SWITCH,
            [Note(0, 0.5 * DEFAULT_SAMPLE_RATE), Note(1, 0.6 * DEFAULT_SAMPLE_RATE)],
            0,
        ),
        Segment(
            TWO_STACK,
            [Note(1, 0.6 * DEFAULT_SAMPLE_RATE), Note(1, 0.7 * DEFAULT_SAMPLE_RATE)],
            0,
        ),
        Segment(
            SWITCH,
            [Note(1, 0.7 * DEFAULT_SAMPLE_RATE), Note(0, 0.8 * DEFAULT_SAMPLE_RATE)],
            0,
        ),
        Segment(
            TWO_STACK,
            [Note(0, 0.8 * DEFAULT_SAMPLE_RATE), Note(0, 0.9 * DEFAULT_SAMPLE_RATE)],
            0,
        ),
        Segment(
            LONG_INTERVAL,
            [Note(0, 0.9 * DEFAULT_SAMPLE_RATE), Note(1, 5 * DEFAULT_SAMPLE_RATE)],
            0,
        ),
    ]
    groups = MapPatterns().identify_patterns(patterns)
...
def test_skewed_circles_with_invalid_zigzag_is_other():
    patterns = [
        ...
        Segment(
            TWO_STACK,
            [Note(0, 1.2 * DEFAULT_SAMPLE_RATE), Note(0, 1.4 * DEFAULT_SAMPLE_RATE)],
            0,
        ),
    ]
    groups = MapPatterns().identify_patterns(patterns)
...
def test_skewed_circles_valid():
    patterns = [
        ...
        Segment(
            TWO_STACK,
            [Note(0, 0.6 * DEFAULT_SAMPLE_RATE), Note(0, 0.8 * DEFAULT_SAMPLE_RATE)],
            0,
        ),
    ]
    groups = MapPatterns().identify_patterns(patterns)
...
def test_skewed_circles_valid_multi_zigs():
    patterns = [
        ...
        Segment(
            ZIG_ZAG,
            [
                Note(0, 0.8 * DEFAULT_SAMPLE_RATE),
                Note(1, 1 * DEFAULT_SAMPLE_RATE),
                Note(0, 0.6 * DEFAULT_SAMPLE_RATE),
            ],
            0,
        ),
...
def test_even_circles_into_nothing_but_theory():
    ...
    assert groups[0].pattern_name == EVEN_CIRCLES
    assert len(groups[0].segments) == 4
    assert groups[1].pattern_name == NOTHING_BUT_THEORY
...
def test_skewed_circles_into_even_circles():
    ...
    assert groups[0].pattern_name == SKEWED_CIRCLES
    assert len(groups[0].segments) == 3
    assert groups[1].pattern_name == EVEN_CIRCLES
    assert len(groups[1].segments) == 3
